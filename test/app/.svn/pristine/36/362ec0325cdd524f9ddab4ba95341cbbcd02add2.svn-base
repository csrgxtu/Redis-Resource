...
module.exports = {
  ...
  getUnfinishZaiQing: function(req,res){
    ...
    if(pageIndex == null || pageIndex == '' || pageSize == null || pageSize == '')
      return res.send({code: 403,msg: 'pageIndex pageSize ',data: ''});
    if(startTime == null || startTime == "")
      startTime = "2015-01-01 00:00:00";
    if(endTime == null || endTime == "")
    ...
    var sql = "select ZaiQingId,ZaiQingName,Baidu_Lon as Lon,Baidu_Lat as Lat,Address from xfjwt_ZaiQing where Status not in ('结案','完成') "
            + "and SendTime > '" +startTime+ "' and SendTime < '" +endTime+ "' order by SendTime desc limit "+start+","+pageSize+"";
    var sqlTotal = "select count(*) as total from xfjwt_ZaiQing where Status not in ('结案','完成') "
            + "and SendTime > '" +startTime+ "' and SendTime < '" +endTime+ "' ";
    console.log('sql = ' + sql);
    XFJWT_ZaiQing.query(sqlTotal,function(err,sqlTotalResult){
      if(err)
        return res.send({code: 500,msg: 'databases err',data: err});
      if(sqlTotalResult.length == 0)
        return res.send({code: 200,msg:'no data',data: ''});
      XFJWT_ZaiQing.query(sql,function(err,result){
        if(err)
          return res.send({code: 500,msg: 'databases err',data: err});
        return res.send({code: 200,msg: 'successfully',total: sqlTotalResult[0].total,data: result});
      })      
    })
  },
  getZaiQingByUser: function(req,res){
    var userId = req.session.loginInfo.userInfo.UserId;
    var userZQInfoSql = "select zq.ZaiQingId,if(zq.ZaiQingName,zq.ZaiQingName,zq.Address) as ZaiQingName from xfjwt_ZaiQingDiaoPai dp inner join xfjwt_ZaiQing zq "
                      + "on zq.ZaiQingId = dp.ZaiQingId where dp.DiaoPaiDuiXiangId = '"+userId+"' "
                      + "and zq.Status not in ('结案','完成')";
    console.log("userZQInfoSql = " + userZQInfoSql);
    XFJWT_ZaiQing.query(userZQInfoSql,function(err,result){
        if(err)
          return res.send({code: 500,msg: 'databases err',data: err});
        return res.send({code: 200,msg: 'successfully',data: result});
    })
