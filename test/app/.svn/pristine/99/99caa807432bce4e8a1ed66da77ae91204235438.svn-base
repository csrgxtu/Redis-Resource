 ...
 *   返回lat，lon，等相关信息 
 */
function getFirePositionByTask(pageNumber, pageSize){
	$.ajax({
		type:"post",
		...
		success:function(msg){
			if(msg.code == 200){
				...
				$("#jieJingListTable").datagrid("loadData",new dataGrid(msg.total,fires));		
...
$(function(){
	getGpsInfoInServer();
	getFirePositionByTask(pageNumber, pageSize);
	initDataGridPage();
});
function initDataGridPage(){
	var p = $('#jieJingListTable').datagrid('getPager');
	$(p).pagination({
		beforePageText: '第',
